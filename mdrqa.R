mdrqa <- function(data,emb,del,norm,rad) {
  ...
  output <- list(SizeRP = RESULTS[1], REC = RESULTS[2], RP=RP, DET = RESULTS[3], ADL = RESULTS[4], MDL = RESULTS[5], DENTR = RESULTS[6], LAM = RESULTS[7], AVL = RESULTS[8], MVL = RESULTS[9], VENTR = RESULTS[10], DIM = PARAMETERS[1], EMB = PARAMETERS[2], DEL = PARAMETERS[3], RAD = PARAMETERS[4], NORM = PARAMETERS[5])#, RP = RP)
